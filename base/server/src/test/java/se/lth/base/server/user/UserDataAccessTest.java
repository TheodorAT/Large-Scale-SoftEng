...
public class UserDataAccessTest extends BaseDataAccessTest {
    ...
    public void addNewUser() {
        userDao.addUser(new Credentials("Generic", "qwerty", Role.USER, "User", "User", "GenericUser@user.se", "+4600000001"));
    ...
    public void addDuplicatedUser() {
        userDao.addUser(new Credentials("Gandalf", "mellon", Role.USER, "User", "User", "user@user00.se", "+4600000001"));
        userDao.addUser(new Credentials("Gandalf", "vapenation", Role.USER, "User", "User", "user@user01.se", "+4600000001"));
    }
    ...
    public void removeUser() {
        User user = userDao.addUser(new Credentials("Sven", "a", Role.ADMIN, "User", "User", "user@user3.se", "+4600000001"));
    ...
    public void authenticateNewUserTwice() {
        ...
        Session authenticated = userDao.authenticate(new Credentials("Elin", "password", Role.NONE, "User", "User", "user@user8.se", "+4600000001"));
        assertNotNull(authenticated);
        assertEquals("Elin", authenticated.getUser().getName());

        Session authenticatedAgain = userDao.authenticate(new Credentials("Elin", "password", Role.NONE, "User", "User", "user@user9.se", "+4600000001"));
    ...
    public void removeSession() {
        userDao.addUser(new Credentials("MormorElsa", "kanelbulle", Role.USER, "User", "User", "user@user9.se", "+4600000001"));
        Session session = userDao.authenticate(new Credentials("MormorElsa", "kanelbulle", Role.NONE, "User", "User", "user@user10.se", "+4600000001"));
    ...
    public void failedAuthenticate() {
        userDao.addUser(new Credentials("steffe", "kittylover1996!", Role.USER, "User", "User", "user@user11.se", "+4600000001"));
        userDao.authenticate(new Credentials("steffe", "cantrememberwhatitwas! nooo!", Role.NONE, "User", "User", "user@user12.se", "+4600000001"));
    }
    ...
    public void updateMissingUser() {
        userDao.updateUser(10, new Credentials("admin", "password", Role.ADMIN, "User", "User", "user@user17.se", "+4600000001"));
    }

    @Test
    public void updateUser() {
        User user = userDao.updateUser(2, new Credentials("test2", "newpass", Role.USER, "User", "User", "user@user18.se", "+4600000001"));
