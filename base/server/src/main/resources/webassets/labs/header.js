...
const baseLab = {
      ...
      .then(function (headerHTML) {
        ...
        document.querySelectorAll('a.anchor:not([name="Top"])').forEach(function (mainAnchor, i) {
          ...
          mainAnchor.parentElement
            .querySelectorAll("a.sub-anchor")
            .forEach((sub, j) => fixLink(i + 1 + ".", sub, "h3", j));
        });
        if (previousUrl) {
  ...
  complete: function (taskId, value) {
    ...
    t.querySelectorAll(".lab-task-form input").forEach((i) => {
      i.disabled = true;
    });
    t.querySelectorAll(".lab-task-form button").forEach((b) => {
  ...
  clearAll: function () {
    Array.from(document.querySelectorAll(".lab-task-group")).map(function (t) {
