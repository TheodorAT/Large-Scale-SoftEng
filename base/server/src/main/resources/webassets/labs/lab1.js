  ...
  base.rest
    ...
    .then(function (data) {
      if (data.error) {
        ...
        baseLab.complete("e2eServer", false);
        alert(
          "There is no data added to the foo table for the user you have logged in with. " +
        ...
        baseLab.complete("e2eServer", false);
        alert(
          "Could not find not total field in the response from http://localhost:8080/rest/foo." +
...
const e2eTabHtmlValidate = function () {
  const p1 = fetch("/hello/hello.html")
...
const e2eTabRouteValidate = function () {
  const p1 = fetch("/index.html")
    .then((response) => response.text())
    .then(function (html) {
      const d = document.createElement("div");
      d.innerHTML = html;
      if (
        Array.from(d.querySelectorAll("a.nav-link"))
          .map((a) => a.href)
